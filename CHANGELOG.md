...
## Unreleased
- [Fix] Hash the full key bytes when computing a partition in `Bindings.partitioner`. The key length was passed as `str.size` (character count) while the key pointer holds the UTF-8 byte sequence, so for a multibyte or binary `partition_key` librdkafka hashed only a truncated prefix - selecting a different partition than other Kafka clients and breaking key-based ordering across multibyte keys. The byte count (`str.bytesize`) is now passed. ASCII keys are unaffected; multibyte/binary keys may now map to a different (correct, client-consistent) partition than before.
