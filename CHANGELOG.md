...
## Unreleased
- [Enhancement] Add `Rdkafka::Config.partitioner_key_uses_bytesize` to opt into hashing the partition key by its byte length in `Bindings.partitioner`. The key length was passed as `str.size` (character count) while the key pointer holds the UTF-8 byte sequence, so for a multibyte or binary `partition_key` librdkafka hashed only a truncated prefix - selecting a different partition than other Kafka clients and breaking key-based ordering across multibyte keys. Passing the byte count (`str.bytesize`) is correct, but changes the partition a given multibyte/binary key maps to, so it is gated behind this flag which **defaults to the previous character-length behavior** to avoid a breaking change; set it to `true` to get the correct behavior. ASCII keys are unaffected either way. The default is expected to flip in a future major version.
