...
## Unreleased
- [Fix] Redesign `NativeKafka` synchronization to remove a counter race and a close deadlock. The in-progress operations counter was incremented under one mutex and decremented under another, so concurrent updates could be lost - letting `#synchronize` (used by `close`) destroy the handle while an operation was still using it (crash, lost messages) or leaving the counter stuck above zero (hung close). Separately, `close` held exclusive access while joining the polling thread, so a callback running on that thread (a delivery report, or the documented oauthbearer token refresh) that re-entered `with_inner` blocked on the same mutex and deadlocked close permanently. The counter is now mutated under a single mutex with a condition variable (so `#synchronize` releases the mutex while draining), and `close` joins the polling thread before taking exclusive access.
