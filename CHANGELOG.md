...
## Unreleased
- [Enhancement] Update librdkafka to `2.15.0`. The statistics filter patch was rebased (upstream grew the config property index; the patch keeps extending it to `64 * 40`).
- [Feature] Add preview support for KIP-932 share groups ("Queues for Kafka") via `Rdkafka::ShareConsumer`, created with `Config#share_consumer`. Share groups let multiple members consume the same partitions cooperatively with per-record acknowledgements (`:accept`, `:release`, `:reject`) instead of committed offsets. The API surface mirrors the librdkafka preview: `#subscribe`/`#unsubscribe`/`#subscription`, batch `#poll` (returning `ShareConsumer::Message` objects that carry `delivery_count` and record-level `error`), `#acknowledge` (by message coordinates), `#commit_sync`/`#commit_async`, `#acknowledgement_commit_callback=`, share-specific OAuth token setters and `#close`. There is no background polling thread: the handle is single-threaded by design (librdkafka rejects cross-thread use) and log, statistics and error callbacks are all serviced from within `#poll`. Requires a broker with share groups enabled (Apache Kafka 4.2.0+); librdkafka marks the feature as preview and not production-ready.
- [Enhancement] `RdkafkaError.build_from_c` now uses the human readable string librdkafka attaches to the `rd_kafka_error_t` (`rd_kafka_error_string`, which itself falls back to the generic per-code description) instead of always using `rd_kafka_err2str`, improving diagnostics for every error-pointer path (share consumer operations, producer transactions, admin incremental alter configs). It also destroys a code-0 error object instead of leaking it on the early return.
- [Enhancement] Extract the OAuthBearer token buffer/extension plumbing in `Helpers::OAuth` around two private native-call hooks so the share consumer overrides only the hooks instead of duplicating both public methods.
- [Fix] Make `ShareConsumer#close` safe against in-flight calls from other threads: closing now waits for operations in progress (mirroring `NativeKafka`), preventing a NULL handle from reaching librdkafka (segfault) or a double destroy when two threads raced `close`.
- [Fix] Keep the `ShareConsumer` native handle (and its GC finalizer) when `close`/`destroy` report an error: librdkafka leaves the instance intact in that case (e.g. closing from inside the acknowledgement commit callback), so dropping the references beforehand leaked the client permanently with no retry possible. `close` is now retriable after such an error.
- [Fix] Pin the acknowledgement commit callback `FFI::Function` in the state holder shared with the `ShareConsumer` GC finalizer. When an unclosed consumer was garbage collected, the finalizer-driven destroy flushes pending acknowledgements through the registered callback, whose native trampoline could already have been freed in the same GC cycle (use-after-free).
- [Fix] Raise `ArgumentError` for `ShareConsumer#subscribe` without topics: librdkafka treats an empty share subscribe as an unsubscribe and returns no error (unlike the regular consumer, where it is rejected with `invalid_arg`), silently dropping the subscription.
- [Fix] Capture the librdkafka client name (e.g. `rdkafka#consumer-1`) onto `ShareConsumer#name` from the OAuthBearer token refresh callback. The share handle has no native name accessor, which made the documented name-based routing of the global oauth callback impossible for share consumers in multi-client processes.
...
- [Fix] Stop the `share_consumer_multi_member` integration spec from failing on redelivery. It asserted every record was consumed exactly once across the group, but KIP-932 share groups are documented (librdkafka's `rdkafka.h`, "Share groups" section) as at-least-once: a record whose acquisition lock expires before it is acknowledged becomes available again and may be redelivered, possibly to a different member with an incremented `delivery_count` - expected broker behavior under a slow/contended poll cadence, not a client bug. The spec now tracks distinct payloads instead of a gross delivery count (so a redelivery cannot end the loop before every record has been seen at least once) and fails only when a record is never delivered, reporting any redelivery as an informational note.
- [Fix] Enable KIP-932 share groups on the macOS CI broker. The macOS jobs stand up their own Confluent KRaft broker from `server.properties` (the Linux jobs use docker-compose, which was already updated), but that config never opted into the share rebalance protocol, so the one broker-backed share consumer spec (`consumes produced messages with delivery counts`) received zero records and failed on every macOS runner. The KRaft config now appends `group.coordinator.rebalance.protocols=classic,consumer,share` and single-replica share-coordinator state-topic settings, and `CONFLUENT_VERSION` is bumped to `8.3.0` (Apache Kafka 4.2.0+, matching the docker-compose broker) since share groups require it; the download path is now derived from the version so a Renovate bump cannot desync it.
- [Enhancement] Add two share consumer integration specs: `share_consumer_implicit_ack_spec` (default implicit acknowledgement mode - polling alone accepts the previous batch, and once accepted records are not redelivered to a second member of the group) and `share_consumer_multi_topic_spec` (a single share consumer subscribed to several topics at once receives every topic's records, attributed to the right topic). Both track the first delivery of each distinct payload and break on the distinct count rather than a gross received count, so an at-least-once redelivery cannot end the consume loop before every record has arrived or fail the assertions (matching the `share_consumer_multi_member` fix); a redelivery is reported as an informational note.
- [Fix] Stop the `statistics_unassigned_producer` integration spec from flaking on a metadata-propagation race. It creates a 1000-partition topic and immediately produces to it, but the create/metadata waits only cover the admin client - the freshly built producer client can still send before it has fetched the new topic's metadata and the delivery fails with `unknown_topic_or_part` (or `leader_not_available` before the partition leaders are elected). The produce is now retried on those transient conditions.
- [Note] Share consumers emit the regular consumer statistics JSON (including `cgrp`; there is no share-specific section in librdkafka 2.15.0) through the usual `statistics_callback`, serviced from within `ShareConsumer#poll`. Verified empirically: the share fetch path bypasses the regular per-partition stats bookkeeping entirely, so the `topics` section carries no partition entries for share consumers regardless of the karafka-only `statistics.unassigned.include` filter - the filter patch needs no share-consumer special-casing. Per-partition share metrics (SPSO/SPEO, in-flight/acquired counts) are not exposed by librdkafka 2.15.0 at all; share-group instrumentation has to be built from `cgrp`, per-message `delivery_count` and the acknowledgement commit callback until upstream adds share stats.
