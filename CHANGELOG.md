...
- [Enhancement] Add two share consumer integration specs: `share_consumer_implicit_ack_spec` (default implicit acknowledgement mode - polling alone accepts the previous batch, and once accepted records are not redelivered to a second member of the group) and `share_consumer_multi_topic_spec` (a single share consumer subscribed to several topics at once receives every topic's records, attributed to the right topic).
