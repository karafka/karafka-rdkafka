...
RSpec.describe Rdkafka::Bindings do
  it "loads librdkafka" do
    expect(described_class.ffi_libraries.map(&:name).first).to include "librdkafka"
  end
  ...
  describe ".lib_extension" do
    it "knows the lib extension for darwin" do
      ...
      expect(described_class.lib_extension).to eq "dylib"
    end

    it "knows the lib extension for linux" do
      stub_const("RbConfig::CONFIG", "host_os" => "linux")
      expect(described_class.lib_extension).to eq "so"
    end
  end
  ...
  describe "error callback" do
    ...
    context "with an error callback" do
      ...
      it "handles fatal error remapping" do
